...
| **LED Anode** | ❤️ Long Leg | **PB7** | Connected via a resistor (Red wire to control signal) |
...
* **LED Polarity:** LEDs only allow current to flow in one direction. Ensure the longer leg (Anode) connects to the signal pin (PB7) and the shorter leg (Cathode) connects to Ground (GND).
